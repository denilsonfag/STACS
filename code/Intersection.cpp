...
#include "Intersection.hpp"
    ...
    Intersection::Intersection(int n, Node* &nodes_vector){
        this->n = n;
        this->nodes_vector = nodes_vector;
    }
    Intersection::~Intersection(){
    }

    // retorna verdadeiro se as arestas cruzarem:
    bool Intersection::verify(int ai, int aj, int bi, int bj){
...
//-----          cout << "  " << ai << " -> " << aj << "  X  " << bi << " -> " << bj << "  =  ";
        ...
        if ((ai == aj) || (ai == bi) || (ai == bj) || (aj == bi) || (aj == bj) || (bi == bj)){
            ...
            if (aux == 0.0){  // se aux = 0.0, as retas são paralelas
                ...
                bool intersect_edge_a = false;
                bool intersect_edge_b = false;

                // verificando se o ponto de intersecção pertence à aresta a:
                double larger_ax;
                double lower_ax;
                if (ai_x > aj_x){ larger_ax = ai_x; lower_ax = aj_x; }
                else{ larger_ax = aj_x; lower_ax = ai_x; }
                double larger_ay;
                double lower_ay;
                if (ai_y > aj_y){ larger_ay = ai_y; lower_ay = aj_y; }
                else{ larger_ay = aj_y; lower_ay = ai_y; }
                ...
                if (intersect_edge_a){ // o ponto de intersecção pertence à aresta a; verificar em b:
                    ...
                    double larger_bx;
                    double lower_bx;
                    if (bi_x > bj_x){ larger_bx = bi_x; lower_bx = bj_x; }
                    else{ larger_bx = bj_x; lower_bx = bi_x; }
                    double larger_by;
                    double lower_by;
                    if (bi_y > bj_y){ larger_by = bi_y; lower_by = bj_y; }
                    else{ larger_by = bj_y; lower_by = bi_y; }
