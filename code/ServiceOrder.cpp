...
void ServiceOrder::new_service(int utx, int uty, int type, int tdispatch, int texec, int deadexec, int kmi, int kme){ 
    ...
    utm_y = uty;
    type_service = type;
    time_dispatch = tdispatch;
    time_execution = texec;
...
void ServiceOrder::set_executed_service(double partial_route_temporal_cost){
    executed = true;
...
void ServiceOrder::set_total_time_execution_real_solution(double partial_route_cost){
...
void ServiceOrder::print_total_time_execution(int s){
    ...
    double diff_time = total_time_execution_real_solution - total_time_execution_proposed_solution;
    double improvement_time = (diff_time / total_time_execution_real_solution) * 100;
...
void ServiceOrder::save_service_order(ofstream& file_out){
    ...
    file_out << "\t\thorario de despacho: " << time_dispatch << "  \ttempo de execução da ordem: " << time_execution << " segundos\r\n";
}
