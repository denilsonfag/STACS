...
#include "LocalSearch.hpp"
    ...
    LocalSearch::LocalSearch(double time_ini_execution, DoubleMatrix* &cost_matrix) {
        depot = DEPOT_INDEX;
    ...
    bool LocalSearch::two_opt(MtspSolution* &solution){
...
//-----                            cout << "--> not changed: neighbor depots\r\n";
                ...
                if (change){
                    ...
                    if (gain > 0.0){  // realizando a troca apenas se o ganho total for positivo
                        ...
                        // copiando nós:
                        for (int i=0; i < size_sol; i++){ new_sol->add(solution->node(i), 0.0); }
    ...
    //-----                    cout << "\r\nsolution  changed:"; new_sol->print();
                        ...
                        if (new_best_cost < current_best_cost){    // a nova solução é melhor, de acordo com o objetivo
                            ...
                            solution->set_time((((double)clock() - time_ini_execution)/CLOCKS_PER_SEC)*1000);  // atualizando o tempo de criação da solução
                            updated_solution = true;
                            
                            #if PRLS
                                cout << "2-opt:\t\t"; solution->print();
                            #endif
                        }
                        else{
...
//-----                            cout << "--> not changed: worst objective cost\r\n";
    ...
    // retorna verdadeiro de atualizar a solução
    bool LocalSearch::three_opt(MtspSolution* &solution){
        bool updated_solution = false;
...
//-----                        if (PDLS) { cout << "--> not changed: invalid cutoff point\r\n"; }
                    ...
                    if (apply){
                        ...
                        if (PDLS) { cout << "\r\n\r\nsolution_1 copied solution:\t"; new_sol_1->print(); }
                        // trocando a <-> b:
                        if (PDLS) {
                            cout << "-> changing    index: " << index_a << " -> " << index_b;
                            cout << "   nodes: " << new_sol_1->node(index_a) << " -> " << new_sol_1->node(index_b) << "\r\n";
                        }
                        new_sol_1->change_nodes(index_a, index_b, cost_matrix);  // troca a posição dos nós e recalcula a solução temporária
                        if (PDLS){ cout << "solution_1  changed:\t\t"; new_sol_1->print(); }
                        // trocando c <-> d:
                        if (PDLS) {
                            cout << "-> changing    index: " << index_c << " -> " << index_d;
                            cout << "   nodes: " << new_sol_1->node(index_c) << " -> " << new_sol_1->node(index_d) << "\r\n";
                        }
                        ...
                        if (PDLS) { cout << "\r\nsolution_2 copied solution:\t"; new_sol_2->print(); }
                        // trocando a <-> c:
                        if (PDLS) {
                            cout << "-> changing    index: " << index_a << " -> " << index_c;
                            cout << "   nodes: " << new_sol_2->node(index_a) << " -> " << new_sol_2->node(index_c) << "\r\n";
                        }
                        new_sol_2->change_nodes(index_a, index_c, cost_matrix);  // troca a posição dos nós e recalcula a solução temporária
                        if (PDLS){ cout << "solution_2  changed:\t\t"; new_sol_2->print(); }
                        // trocando b <-> d:
                        if (PDLS) {
                            cout << "-> changing    index: " << index_b << " -> " << index_d;
                            cout << "   nodes: " << new_sol_2->node(index_b) << " -> " << new_sol_2->node(index_d) << "\r\n";
                        }
                        ...
                        if (PDLS) { cout << "\r\nsolution_3 copied solution_2:   "; new_sol_3->print(); }
                        
                        // trocando a <-> b:
                        if (PDLS) {
                            cout << "-> changing    index: " << index_a << " -> " << index_b;
                            cout << "   nodes: " << new_sol_3->node(index_a) << " -> " << new_sol_3->node(index_b) << "\r\n";
                        }
                        new_sol_3->change_nodes(index_a, index_b, cost_matrix);  // troca a posição dos nós e recalcula a solução temporária
                        if (PDLS){ cout << "solution_3  changed:\t\t"; new_sol_3->print(); }
                        ...
                        if (PDLS) { cout << "\r\nsolution_4 copied solution_2:   "; new_sol_4->print(); }
                        
                        // trocando c <-> d:
                        if (PDLS) {
                            cout << "-> changing    index: " << index_c << " -> " << index_d;
                            cout << "   nodes: " << new_sol_4->node(index_c) << " -> " << new_sol_4->node(index_d) << "\r\n";
                        }
                        new_sol_4->change_nodes(index_c, index_d, cost_matrix);  // troca a posição dos nós e recalcula a solução temporária
                        if (PDLS) { cout << "solution_4  changed:\t\t"; new_sol_4->print(); }
    ...
    double LocalSearch::gain_change_two_nodes(int index_a, int index_b, MtspSolution* &solution){
    ...
    double LocalSearch::partial_cost(int before, int current, int after, MtspSolution* &solution){
